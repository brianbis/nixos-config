...
mv -f "$out/steam-pattern.tmp" "$out/steam-pattern"
{
  ...
  echo "hushmic_proc: $(pgrep -af hushmic 2>/dev/null | grep -vE 'hushmic-probe|hushmic-cpu-watcher|hushmic-cpu-mode|pgrep' | tr '\n' '|')"
  echo "watchers: $(ps -eo pid,etime,comm,args 2>/dev/null | grep -E 'steam-game-watcher|audio-cpu-watcher|hushmic-cpu-watcher' | grep -v grep | tr '\n' '|')"
  echo "steam_match: $(find /proc -maxdepth 2 -type f -name cmdline -exec grep -lzE -f "$out/steam-pattern" {} + 2>/dev/null | tr '\n' ' ')"
  echo "unit_state: boost=$(systemctl is-active hushmic-cpu-boost 2>/dev/null) probe=$(systemctl is-active hushmic-probe 2>/dev/null)"
  echo "journal: $(journalctl --no-pager -n 200 -u hushmic-cpu-boost -u hushmic-probe 2>/dev/null | grep -E 'Started|Stopped|Deactivated|Activating|Main process' | tail -3 | tr '\n' '|')"
  ...
  echo "sched: $(for p in $(pgrep -f 'hushmic-wrapped|filter-chain|pipewire' 2>/dev/null | sort -u); do printf '[%s]pol=%s rtlim=%s ' "$p" "$(awk '/^Sched:/{print $2}' /proc/$p/status 2>/dev/null)" "$(awk '/Max real-time priority/{print $4}' /proc/$p/limits 2>/dev/null)"; done)"
